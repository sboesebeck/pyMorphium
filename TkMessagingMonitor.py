...
try:
    opts, args = getopt.getopt(argv, "?r:d:h:p:A:c:a:pt:",
                               ["port=", "rows=", "types=", "additional=", "host=", "database=", "collection=",
                                "filter="])
except getopt.GetoptError as e:
    print(e)
    print(
        'TkMessagingMonitor.py -h|--host=<host> -d|database=<dbname> -c|collection=<collection> -a <ADDITIONAL Field> --filter=key:value --types=nlpda')
    sys.exit(2)
for opt, arg in opts:
    if opt == '-?':
        print(
...
title = Label(root, text="Messaging Monitor: %s:%i %s/%s" % (host, port, dbname, collection), relief=RAISED,
              bg="#a0a0f0").grid(row=1, column=0, columnspan=columnCount, pady=5, sticky=W + E)
    ...
    try:
        with col.watch([{'$match': {'ns.db': dbname, 'ns.coll': collection}}], 'updateLookup') as stream:
            for insert_change in stream:
                ...
                unprocessed = col.count_documents({"processed_by": None})
