...
def main(argv):
    ...
    answerTimes = False
    try:
        opts, args = getopt.getopt(argv, "?sAh:m:d:c:a:pt:",
                                   ["answertimes", "stats", "types=", "additional=", "host=", "database=",
                                    "collection=", "filter="])
    ...
    try:
        with col.watch([{'$match': {'ns.db': dbname, 'ns.coll': collection}}], 'updateLookup') as stream:
            for insert_change in stream:
                ...
                if ((num % modulo) == 1):
                    ...
                    if stats:
                        ...
                        if answerTimes:
                            ...
                            print(
                                '      answers registered {} - sum answering time {} - Avg. Answer time {} - {} unanswered messages'.format(
                                    answers, answerTimesSum, avgAnswerTime, len(messages)))
                    print('{:>15} {:>5} - {:<50} {:<45} {:<25} {:<25} {:<38} {:<38} {:<6} {:<25} - {}'.format("time",
                                                                                                              "num#",
                                                                                                              "msgType",
                                                                                                              "name",
                                                                                                              "msg",
                ...
                print('{:>15} {:>5} - {:<50} {:<45} {:<25} {:<25} {:<38} {:<38} {:<6} {:<25} - {}'.format(
                    int(round(time.time() * 1000)), num, msgType, name, msg, msgId, sender, recipient, exclusive,
                    inAnswerTo, add))
                # print(insert_change)
                # print("")
